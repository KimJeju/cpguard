...
Read it plainly: **Java and Python are at working-tool quality; JS/TS, Ruby and PHP are usable; Go, C#, C and C++ are not there yet.** Precision tells a different story from score — NestJS 83.1%, Koa 81.0% and C# 87.8% are close to Java's 88.4%. Where the score is low it is usually recall, not noise (C# recall is 30.0%). Kotlin and Swift have no public labelled corpus at all, so they are covered by idiom tests instead (see below).
...
**The benchmarks drove real engine work.** Java started at 0.137 and PHP at 0.001; every point since came from a defect the numbers exposed — taint dying inside `try` blocks, for-each variables losing the collection's taint, no path sensitivity at all, container mutations not propagating, comparison results carrying taint, constructor-to-field-to-method flows missing in every language, by-reference output parameters (`fgets(buf, …)`, `Decode(&v)`) invisible to a return-value-only model, closures running against an empty environment so anything they captured looked clean, and — most recently — validation applied to a *projection* of the tainted value (`if !allowed[url.Parse(x).Hostname()] { reject }`) leaving the original tainted, which was 59% of all Go false positives. Each fix is a general analysis technique, not a tweak aimed at the test cases. The full before/after ledger is in [`bench/README.md`](bench/README.md).
