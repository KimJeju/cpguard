...
PDF_UI: dict[str, str] = {
    ...
    "조치 방안": "Remediation", "취약한 코드 예시": "Vulnerable Code Example",
    "안전한 코드 예시": "Safe Code Example", "참고": "References",
...
REMEDIATION_EN = {
    ...
    "deserialization": ("Insecure Deserialization",
             "Untrusted data is deserialized into objects, which can lead to arbitrary code execution.",
             "Restrict deserializable classes to an allowlist, or switch to a data-only format such as JSON, and verify signatures and integrity.",
             "new ObjectMapper().readValue(body, Dto.class)"),
    "ldap-injection": ("LDAP Injection",
             "User input is concatenated into an LDAP search filter, allowing the query criteria to be altered.",
             "Escape filter values per RFC 4515 and prefer an API that supports binding.",
             "(uid= + escapeLDAP(user) + )"),
    "xpath-injection": ("XPath Injection",
             "User input is concatenated into an XPath expression, allowing the query structure to be changed.",
             "Use variable binding (e.g. XPathVariableResolver) and remove string concatenation.",
             "xpath.setXPathVariableResolver(v);  /user[@id=$id]"),
    "buffer-overflow": ("Buffer Overflow",
             "A copy function without length checking can overwrite adjacent memory.",
             "Replace strcpy/strcat/sprintf/gets with length-bounded functions and check the destination buffer size explicitly.",
             "snprintf(dst, sizeof(dst), \"%s\", src)"),
    "format-string": ("Format String",
             "User input is used as the format string itself, enabling memory disclosure or corruption.",
             "Keep the format string constant and pass user input only as an argument.",
             "printf(\"%s\", user)"),
    "library-injection": ("Library Injection",
             "User input is used as a dynamic library load path, allowing an arbitrary module to be loaded.",
             "Pin the load path to an absolute allowlisted location and verify the signature before loading.",
             "dlopen(\"/opt/app/lib/plugin.so\", RTLD_NOW)"),
    "intent-redirect": ("Intent Redirection",
             "An externally supplied Intent is re-dispatched without validation, allowing private components to be invoked.",
             "Do not forward a received Intent as is; validate the target component against an allowlist.",
             "if (target.component in ALLOWED) startActivity(target)"),
    "webview": ("Unsafe WebView Configuration",
             "JavaScript execution, file access or a JS bridge is enabled, letting remote content run with app privileges.",
             "Disable JavaScript and file access when not needed, and expose a JS bridge only to trusted local content.",
             "settings.allowFileAccess = false"),
    "token-storage": ("Auth Token in Web Storage",
             "Tokens kept in localStorage or sessionStorage are stolen outright by a single XSS.",
             "Move tokens to HttpOnly, Secure, SameSite cookies so scripts cannot read them.",
             "Set-Cookie: token=...; HttpOnly; Secure; SameSite=Lax"),
    "cookie-flags": ("Missing Cookie Security Attributes",
             "Without Secure, HttpOnly and SameSite the cookie is exposed to cleartext transport, script access and CSRF.",
             "Set Secure, HttpOnly and SameSite on session cookies, and use the __Host- prefix where applicable.",
             "res.cookie('sid', v, {secure:true, httpOnly:true, sameSite:'lax'})"),
    "debug": ("Debug Code or Settings Left in Place",
             "Debug output or debug mode left in production exposes internal details and stack traces.",
             "Turn debug flags off before release, remove debug output, and replace error pages with generic messages.",
             "DEBUG = False"),
    "comment-leak": ("Information Disclosure in Comments",
             "Comments retain accounts, internal paths or open issues that can aid an attacker.",
             "Strip comments containing internal information from release artifacts and move open items to an issue tracker.",
             "// see issue tracker"),
    "hardcoded-connection": ("Hardcoded Connection Details",
             "Database, mail or directory connection strings and accounts are embedded in source and are immediately abusable if leaked.",
             "Move connection details to environment variables or a secret manager and rotate any exposed account.",
             "url = os.environ['DB_URL']"),
    "default-account": ("Default or Test Account",
             "Default accounts such as admin or test with guessable passwords remain in the source.",
             "Remove default and test accounts; issue production accounts under the password policy and least privilege.",
             "user = os.environ['APP_USER']"),
    "dev-path": ("Developer Local Path Disclosure",
             "Absolute paths from a developer machine reveal internal structure and user names and break in production.",
             "Replace paths with configuration values or relative paths and strip absolute paths from build artifacts.",
             "BASE = os.environ.get('APP_HOME', '/opt/app')"),
    "dependency": ("Open-Source Component with Known Vulnerabilities",
             "A third-party component in use has a published vulnerability (CVE), exposing that weakness directly in the service.",
             "Upgrade to a fixed version. If an immediate upgrade is not possible, restrict exposure of the affected functionality and maintain an SBOM to prevent recurrence.",
             "// pin the component to a fixed version in package.json / pom.xml"),
    "internal-info": ("Internal Configuration Disclosure",
             "Private IPs and admin paths are hardcoded in source and are useful for reconnaissance.",
             "Move internal addresses and admin paths to configuration, and protect admin screens with access control such as allowlisted IPs and authentication.",
